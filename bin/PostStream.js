...
module.exports = (_temp = _class = class PostStream extends events {
    // if user has registered data event

    /**
     * receive special data
     * @type {EventEmiter}
     */
    //0: fixed length，1：unfixed length(use _endFlag to finding end)
    //string
    // Buffer //when received data length too short，can`t decode header，the data will be store here
    // stream.Writable | stream.Duplex
    constructor(readable, writable) {
        ...
        this._emitDataEvent = false;
        this.data = new events();
        this.parseData = true;
        if (readable != null) {
            if (readable.__PostStreamUsed === true) throw new Error('stream has been used by PostStream');
        ...
        if (writable != null) {
            if (writable.__PostStreamUsed === true) throw new Error('stream has been used by PostStream');
        ...
        this.on('newListener', event => {
            if (event === 'data') this._emitDataEvent = true;
        });
    }
    ...
    _sortData(data) {
        ...
        if (this._mode === 0) {
            if (data.length >= this._remainBodyLength) {
                ...
                if (this._emitDataEvent) this.emit('data', this._receivedTitle, ...body);

                this.data.emit(this._receivedTitle, ...body);

                if (data.length > this._remainBodyLength) this._sortData(data.slice(this._remainBodyLength));
            } else {
            ...
            if (index === -1) {
                ...
                this._new = true;
                if (this._emitDataEvent) this.emit('data', this._receivedTitle, this._receivedBody);
...
}, _class._endFlag = Buffer.from('\r\n«end»'), _class.serialize = serialize, _class.parse = parse, _temp);