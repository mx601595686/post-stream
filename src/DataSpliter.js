...
exports.DataSpliter = class DataSpliter extends EventEmiter {
    ...
    constructor(readable, config = {}) {
        super();
        if ('number' === typeof config.maxSize)
    ...
    _push = (data) => {
        try {
            this._findStartAndConcatData(data) &&
            this._getHeader() &&
            this._getBody() &&
            this._clean();
        } catch (e) {
    ...
    _findStartAndConcatData(data) {
        if (this._receivedFragment === undefined) {  //find new data fragment
            ...
            if (length > this.maxSize) { //if greater than maxSize,ignore this data fragment
        ...
        switch (mode) {
            ...
            case 1: {
                ...
                if (index !== -1) {
                    ...
                    if (index + endFlag.length < data.length) { //remain data
